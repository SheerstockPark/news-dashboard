...
cloud checkout of SheerstockPark/news-dashboard. Produce and DELIVER the **Morning** briefing.
...
a) Save the prose to /tmp/brief.md, then:
   `gh workflow run briefing.yml --repo SheerstockPark/news-dashboard --ref main -f edition=Morning -F brief_text=@/tmp/brief.md`
b) If dispatch fails (no gh, no permission): write the prose to `briefs/pending-morning.md`,
   then `git add briefs/pending-morning.md && git commit -m "Routine: morning brief $(date -u +%F)" && git push origin main`.
