...
from newsdash import alerts, ingest  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--min-relevance", type=int, default=70)
    ap.add_argument("--min-impact", type=int, default=60)
    ap.add_argument("--keywords", nargs="*", default=[])
    args = ap.parse_args()

    if alerts.channel() == "none":
    ...
    def check():
        if args.fetch:
            ingest.run_once()
        s = alerts.run_once(args.min_relevance, args.min_impact, args.keywords,
